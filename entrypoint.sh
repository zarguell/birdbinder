...
# Start uvicorn and a supervised huey consumer in the background
echo "Starting BirdBinder..."

uvicorn app.main:app --host 0.0.0.0 --port 8000 &
UVICORN_PID=$!

# If the consumer dies (OOM, crash), restart it — otherwise card generation
# silently dies while the /api/health check stays green
(
    while true; do
        python -m huey.bin.huey_consumer app.huey_instance --worker-type process --workers 2
        echo "huey_consumer exited ($?); restarting in 2s..." >&2
        sleep 2
    done
) &
HUEY_SUPERVISOR_PID=$!
...
trap "kill $UVICORN_PID $HUEY_SUPERVISOR_PID; wait" EXIT TERM INT
